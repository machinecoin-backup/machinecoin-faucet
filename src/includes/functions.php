...
function get_avarage_payout($db_link)
{
    ...
    $payout_average = (float)0;

    while ($row = mysqli_fetch_array($db_result)) {
        $payout_average = $payout_average + (float)$row['payout'];
    }

    $am = mysqli_num_rows($db_result);

    return (float)($payout_average / $am);  
}
...
function get_daily_payout($db_link)
{
    ...
    $payout_daily = (float)0;

    while ($row = mysqli_fetch_array($db_result)) {
        $payout_daily = $payout_daily + (float)$row['payout'];
    }

    return (float)$payout_daily;
}
...
function get_total_payout($db_link)
{
    ...
    while ($row = mysqli_fetch_array($db_result)) {
        $payout_total = $payout_total + (float)$row['payout'];
    }

    return (float)$payout_total;
}
